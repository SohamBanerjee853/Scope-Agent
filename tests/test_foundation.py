...
def test_session_paths_are_portable_and_contained(session):
    ...
    assert path.stem.startswith(("session-", "sha256-"))
    assert all(char.isalnum() or char in "-_" for char in path.stem)
...
@pytest.mark.parametrize("session", ["", None, 3])
...
def test_logs_utf8_and_incomplete_records():
    ...
    log.append("test", "task_start", description="理解", task_id="id")
    record = log.read("test")[0]
    assert record["fields"] == {"description": "理解", "task_id": "id"}
...
def test_event_validation():
    log.append("test", "data", ts="untrusted field")
    record = log.read("test")[0]
    assert record["ts"] != "untrusted field"
    assert record["fields"]["ts"] == "untrusted field"
    with pytest.raises(ValueError):
...
def test_help_is_lazy(capsys):
    ...
    assert "scope.watch" not in set(sys.modules) - before
