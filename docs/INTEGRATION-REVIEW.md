...
Integration follow-up: the reviewed direct question route is replaced by the
watcher-only helper in the reconciled tree. The diagnostic below now exercises
the real tagged watcher question and also forbids any caller-side terminal read.
It reports `late_answer_rejected` after confirmed shared revoke. The original
finding and revision references remain below as historical evidence; see the
[reconciliation checkpoint](RECONCILIATION-CHECKPOINT.md) for current validation.
